  ...
  var ispresent = ()=>{
    if(this.cache[dir].indexOf(file) != -1){
      ...
      if(candidates.length>0){
        ...
        return this.defaultIcon(file,callback);
      }
...
Theme.prototype.defaultIcon = function(file,callback){
  var pixmaps = "/usr/share/pixmaps";
  var test = ()=>{
    var i =-1;
    if((i = this.cache[pixmaps].indexOf(file)) != -1){
      callback(null, path.join(pixmaps,this.cache[pixmaps][i]));
    }else{
    }

  }
  if(!this.cache[pixmaps]){
    fs.readdir(pixmaps,(err,files)=>{
      if(!err){
        this.cache[pixmaps] = files;
      }else{
        this.cache[pixmaps] = [];
      }
      test();
